window.onload = function() {
    ...
    imageInput.addEventListener("change", function() {
        if (this.files && this.files[0]) { handleFiles(this.files); }
    });

    function handleFiles(files) {
        Potrace.loadImageFromFile(files[0]);
        // Rimuovi eventuali risultati precedenti se ricarichi
        svgDiv.classList.remove('has-result');
        svgDiv.innerHTML = '<div class="placeholder"><img src="img/imgvector.svg" alt="icon"><p>Il risultato vettoriale apparirà qui</p></div>';
    }

    function displaySVG(size) {
        // 1. Pulisci il div
        svgDiv.innerHTML = "";
        
        // 2. Genera e inserisci l'SVG
        var svg = Potrace.getSVG(size);
        svgDiv.insertAdjacentHTML('beforeend', svg);
        
        // 3. Adatta le dimensioni dell'SVG
        var svgElement = svgDiv.querySelector('svg');
        if (svgElement) {
            svgElement.setAttribute("viewBox", `0 0 ${Potrace.img.width * size} ${Potrace.img.height * size}`);
            svgElement.style.width = "100%";
            svgElement.style.height = "auto";
        }

        // 4. MODIFICA: Attiva lo sfondo bianco nell'interfaccia
        svgDiv.classList.add('has-result');
    }
    ...
    saveBtn.onclick = function(e) {
        if (!saveBtn.classList.contains('ready')) {
            e.preventDefault();
            return false;
        }
        // L'export genera l'SVG puro, quindi rimarrà trasparente
        var svgData = Potrace.getSVG(1);
        this.download = "vettoriale-ziply.svg";
        this.href = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(svgData);
    };

    // Drag & Drop logic
    drop.addEventListener("dragover", function(e) { e.preventDefault(); this.classList.add('hovering'); });
    drop.addEventListener("dragleave", function() { this.classList.remove('hovering'); });
...
// ... IL RESTO DEL CODICE POTRACE RIMANE INVARIATO ...

// ... IL RESTO DEL CODICE POTRACE RIMANE INVARIATO ...
// --- DA QUI IN POI IL TUO POTRACE ---
// --- QUI SOTTO LASCIA TUTTO IL TUO CODICE POTRACE ---
// ここからPotrace.js
